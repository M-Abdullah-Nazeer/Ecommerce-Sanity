...
function AddtoBasketButton({ product, disabled }: AddtoBasketButtonProps) {
    ...
    // this ensures that component only renders on client side
    ...
    return (
        <div className="flex items-center justify-center space-x-2">
            ...
            <button onClick={() => addItem(product)}
                className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors duration-200 ${disabled ? "bg-gray-400 cursor-not-allowed" : "bg-maincol hover:bg-hovercol"
                    }`}
